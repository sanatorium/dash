...
*Setup instructions for a Gitian build of Sanity Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sanity
...
- [Building Sanity Core](#building-sanity-core)
...
Clone the git repositories for Sanity Core and Gitian.
...
in the Sanity Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Sanity Core
----------------
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sanity/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/sanatorium/sanity
...
./bin/gbuild --commit sanity=${COMMIT} --url sanity=${URL} ../sanity/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the sanity git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sanity/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/sanity.git
SIGPATH=/some/root/path/sanity-detached-sigs.git

./bin/gbuild --url sanity=${BTCPATH},signature=${SIGPATH} ../sanity/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sanity-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sanity-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sanity-osx-build.assert
```
...
[sanity/gitian.sigs](https://github.com/sanatorium/gitian.sigs/) repository, or if that's not possible to create a pull
request.
