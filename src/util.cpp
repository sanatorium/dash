...
const char * const BITCOIN_CONF_FILENAME = "sanity.conf";
const char * const BITCOIN_PID_FILENAME = "sanityd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sanity" is a composite category enabling all Sanity-related debug output
            if(ptrCategory->count(string("sanity"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SanityCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SanityCore
    // Mac: ~/Library/Application Support/SanityCore
    // Unix: ~/.sanitycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SanityCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SanityCore";
#else
    // Unix
    return pathRet / ".sanitycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sanity.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sanity.conf
