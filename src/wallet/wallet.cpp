...
/**
 * Fees smaller than this (in sanies) are considered zero fee (for transaction creation)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !CPrivateSend::IsDenominatedAmount(pcoin->vout[i].nValue);
                } else if(nCoinType == ONLY_55555) {
                    found = pcoin->vout[i].nValue == 55555 * COIN; // MODMOD: (was 100)
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                    (!IsLockedCoin((*it).first, i) || nCoinType == ONLY_55555) &&
...
bool CWallet::GetMasternodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_55555);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 SANITY."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-sanity-address
                        CScript scriptChange;
